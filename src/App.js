...
class App extends Component {
  ...
  getRandomImg = () => {
    ...
    console.log('Random')
    // Ensure the random image is different from the current image
    while (randomImage.id === imgId) {
      randomImage = imagesList[Math.floor(Math.random() * imagesList.length)]
    }
    this.setState({
      imgUrl: randomImage.imageUrl,
      //   imgAlt: randomImage.category,
      imgId: randomImage.id,
    })
    // this.timerId = setInterval(this.decrementTimer, 1000)
  }

  checkBothImages = id => {
    const {imgId, isGameOver} = this.state
  ...
  selectedTab = tabId => {
    this.setState({type: tabId})
  }

  render() {
    const {score, time, isGameOver, type, imgUrl} = this.state
    const filteredList = imagesList.filter(each => each.category === type)
    const renderTabs = (
      <div className="tabs-thumbnails-container">
        <ul className="tabs-container">
          {tabsList.map(each => (
            <Tab
              each={each}
              isActive={type === each.tabId}
        ...
        <ul className="thumbnails-container">
          {filteredList.map(each => (
            <Thumbnail
              each={each}
              key={each.id}
              checkBothImages={this.checkBothImages}
            />
          ))}
        </ul>
      </div>
        ...
        <ul className="navbar-score-timer-container">
          ...
          <li className="timer-count">
            <img
              ...
              className="timer"
            />
            <p className="paragraph score-value">{time} sec</p>
          </li>
        </ul>
      </nav>
    )
    ...
    return (
      <div className="bg-container">
        {navBar}
        <div className="game-and-score-pages-container">
          {!isGameOver ? (
            renderGamePage
          ) : (
            <GameOver resetGame={this.resetGame} score={score} />
          )}
        </div>
      </div>
    )
